...
- **Runtime owns mechanics, clients own UX.** The store exposes session primitives (`create_session`, `append_session_messages`, `list_sessions`, `session_meta`, `search_sessions`) and the runtime runs auto-compaction only as a context-window safety net. Discretionary lifecycle — `/clear`, `/new`, `/compact`, session selection, archival browsing, saved searches — is composed in the client from those primitives. See [RFC 0207](docs/src/rfcs/0207-store.md).
