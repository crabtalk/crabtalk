...
async fn wx_stream(
    ...
    match acc.session {
        Some(conversation_id) => {
            tracing::info!(agent, chat_id, conversation_id, "stream completed");
            StreamResult::Ok { conversation_id }
        }
        None => {
            tracing::warn!(agent, chat_id, "stream completed without conversation");
            StreamResult::Failed
        }
    }
}
