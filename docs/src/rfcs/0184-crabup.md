...
After this, `crabtalkd`'s CLI is `run` (the long-running process the service unit invokes, equivalent to today's `--foreground`), `reload`, and `events`. Plugin install/uninstall live in crabup as `add`/`remove`, not in the daemon CLI.
