...
> **Superseded by [0205 - Berm](0205-berm.md).**
> `harness/cron` is deleted. Scheduling stops being a service with a timer of
> its own and becomes the harness execution model: a harness asks to be woken,
> the daemon owns one due-set, and a cron entry is the case where the trigger
> is a time. The standalone scheduler is gone before its replacement exists —
> until the due-set lands, a downstream app that needs scheduling writes it
> against the SDK in a few dozen lines.
...
> **Note (2026-08-15).** The crate moved to `harness/cron` when standalone services were split out of `apps/`. Every `apps/cron` below means `harness/cron`; the design is unchanged.
