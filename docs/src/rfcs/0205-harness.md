...
**The guest speaks the daemon's own message types**, which the fallback in this RFC's first draft treated as the likely outcome and the preferred one as unexamined. It is settled: `crates/core`'s module is not separable — it is `std` and carries serde derives — so `harness/proto` generates from the same `crabtalk.proto` a second time. Two configuration differences make it a guest: `btree_map` because `map` fields otherwise reach for `std::collections::HashMap`, and a crate alias because prost writes `::prost::` paths and the workspace's own copy keeps its default features. One schema, two emissions, and no way to drift without the `.proto` changing.
