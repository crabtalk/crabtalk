...
> **Superseded by [0207 (Store)](0207-store.md) (2026-08-18).** `crates/hooks` is deleted, the trait is `Harness`, and the composite is `Hooks`. The lifecycle inverts: there is no agent registry, so `on_register_agent` / `on_unregister_agent` become `on_resolve_agent` / `on_forget_agent`, the first firing per run and required to be idempotent. The membership invariant this RFC promised — hook state in place for every registered agent, dropped when it leaves — no longer exists to be kept.
...
**`usage`** — what this hook's tools are for: when to reach for them and how they go together. Renamed from `system_prompt` (2026-08-16), which named the destination rather than the thing and invited unbounded content. A declaration, not a lifecycle callback. `HarnessHook` supplies its own per-agent through `on_build_agent`, since the trait method has no agent in its signature and would put every harness's text in front of every agent.
