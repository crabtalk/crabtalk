...
(text deltas, tool calls, completions). `DaemonBackend` uses this to broadcast
events to console subscribers.

### Composition

`Env<B: Backend>` is the engine environment. It composes `SkillHandler`,
`McpHandler`, `Memory`, and `AgentScope` maps. It implements `Hook` by
orchestrating all subsystems.

`DaemonEnv` is a type alias: `Env<DaemonBackend>`. The daemon backend
adds ask_user dispatch, delegate dispatch, session CWD, and event broadcasting.

For embedded use, `Env<NoBackend>` provides the full engine without
