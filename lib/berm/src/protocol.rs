...
//! client would and gets the same reply (RFC 0205). Which message types this
//! door carries is checked host-side on decode; one it does not comes back as
...
///
/// Only request-response message types cross this door today, so one reply is
/// the whole answer.
