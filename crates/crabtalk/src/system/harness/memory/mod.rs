//! Memory harness — the `recall` / `remember` / `forget` tools over the
//! [`store::interface::Memory`] interface. Per-tool files own the
//! corresponding handlers. See RFC 0207 for the design.
//!
//! The harness holds a store handle, not a store: entries live in the
...
/// via schemars; this covers everything that doesn't fit in a
/// per-arg description.
const MEMORY_USAGE: &str = "Call `recall` before assuming context is missing. Use \
`remember` for durable facts, `forget` to remove ones no longer true — skip \
transient or one-off details.";

pub struct MemoryHarness<M> {
...
impl<M: Memory + 'static> Harness for MemoryHarness<M> {
